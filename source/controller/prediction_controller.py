...
async def create_data_series(data_series: DataSeries):
    try:
        ...
        session.execute(
            ...
            (
                uuid.uuid4(),
                data_series.id_municipio,
                data_series.valor_indice,
                data_series.data,
            ),
        )
        ...
        print(f"Erro: {e}")
        raise HTTPException(
            status_code=500, detail=f"Erro: {e}, Erro ao criar o registro"
        )
    finally:
        session.shutdown()
        cluster.shutdown()
...
async def list_data_series():
    try:
        ...
        print(f"Erro: {e}")
        raise HTTPException(
            status_code=500, detail=f"Erro: {e}, Erro ao listar registros"
        )
    finally:
        session.shutdown()
        cluster.shutdown()
...
async def get_data_series(data_id: str):
    try:
        ...
        print(f"Erro: {e}")
        raise HTTPException(
            status_code=500, detail=f"Erro: {e}, Erro ao buscar registro"
        )
    finally:
        session.shutdown()
        cluster.shutdown()
